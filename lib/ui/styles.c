...
void init_style_bg_left_MAIN_DEFAULT(lv_style_t *style) {
    lv_style_set_bg_grad_dir(style, LV_GRAD_DIR_HOR);
    lv_style_set_bg_color(style, lv_color_hex(0xff011e48));
    lv_style_set_bg_grad_color(style, lv_color_hex(0xff003685));
    lv_style_set_border_width(style, 0);
    lv_style_set_border_opa(style, 255);
    lv_style_set_outline_opa(style, 0);
};

lv_style_t *get_style_bg_left_MAIN_DEFAULT() {
    static lv_style_t *style;
    if (!style) {
        style = lv_mem_alloc(sizeof(lv_style_t));
        lv_style_init(style);
        init_style_bg_left_MAIN_DEFAULT(style);
    }
    return style;
};

void add_style_bg_left(lv_obj_t *obj) {
    (void)obj;
    lv_obj_add_style(obj, get_style_bg_left_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};

void remove_style_bg_left(lv_obj_t *obj) {
    (void)obj;
    lv_obj_remove_style(obj, get_style_bg_left_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};
...
void init_style_bg_right_MAIN_DEFAULT(lv_style_t *style) {
    lv_style_set_bg_grad_dir(style, LV_GRAD_DIR_HOR);
    lv_style_set_bg_color(style, lv_color_hex(0xff003685));
    lv_style_set_bg_grad_color(style, lv_color_hex(0xff011e48));
    lv_style_set_border_opa(style, 0);
    lv_style_set_border_width(style, 0);
};

lv_style_t *get_style_bg_right_MAIN_DEFAULT() {
    static lv_style_t *style;
    if (!style) {
        style = lv_mem_alloc(sizeof(lv_style_t));
        lv_style_init(style);
        init_style_bg_right_MAIN_DEFAULT(style);
    }
    return style;
};

void add_style_bg_right(lv_obj_t *obj) {
    (void)obj;
    lv_obj_add_style(obj, get_style_bg_right_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};

void remove_style_bg_right(lv_obj_t *obj) {
    (void)obj;
    lv_obj_remove_style(obj, get_style_bg_right_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};
...
void remove_style(lv_obj_t *obj, int32_t styleIndex) {
    ...
    static const RemoveStyleFunc remove_style_funcs[] = {
        ...
        remove_style_bg_left,
        remove_style_bg_right,
    };
